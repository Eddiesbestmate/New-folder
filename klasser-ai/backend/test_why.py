...
def allocate(slots, cls, supply, tsup, cap=CAP):
    ...
    # Walk the cycle once, considering every class at each slot. Running
    # one cohort to completion before starting the next starves whichever
    # goes last - rooms and staff are shared, so the senior years found
    # nothing left. Urgency (periods still owed against slots still to come)
    # keeps a class that cannot afford to wait ahead of one that can.
    ...
    for day in sorted(day_slots):
        for slot in day_slots[day]:
            ...
            def urgency(code):
                room_left = remaining
                return (-(owed[code] / max(1, room_left)), -owed[code], code)
            for code in sorted(all_codes, key=urgency):
                if True:
                    c = cls[code]
                    if owed[code] <= 0 or used_today[day][code] >= cap:
                        continue
    ...
    # For anything short, count why each remaining slot was refused.
    for code in [c for c in cls if owed[c] > 0][:4]:
        c = cls[code]
        why = defaultdict(int)
        for slot in slots:
            if slot in assigned[code]:
                continue
            if c["students"] & slot_students[slot]:
                why["student clash"] += 1; continue
            if sum(1 for s in assigned[code] if s[0] == slot[0]) >= cap:
                why["day cap"] += 1; continue
            if room_use[(c["campus"], c["rt"], slot)] >= supply.get(
                    (c["campus"], c["rt"]), 0):
                why[f"no {c['rt']}"] += 1; continue
            if teach_use[(c["campus"], c["subject"], slot)] >= tsup.get(
                    (c["campus"], c["subject"]), 0):
                why[f"no {c['subject']} teacher"] += 1; continue
            why["FREE"] += 1
        print(f"  {code} ({c['subject']}, {c['campus']}) short {owed[code]}: "
