...
def build_blocks(cls):
    ...
    def bkey(v):
        return ((v["year"], v["campus"], v["periods"]) if v["year"] in JUNIOR
                else (v["year"], None, v["periods"]))
    for key in sorted({bkey(c) for c in cls.values()}, key=lambda k: (k[0], str(k[1]), -k[2])):
        year, campus, periods = key
        members = sorted(c for c, v in cls.items() if bkey(v) == key)
        for code in members:
            for b in blocks:
                if b["key"] != key:
                    continue
...
def allocate(slots, cls, supply, tsup, blocks, cap=CAP):
    ...
    # Phase 1: decide how many periods each block takes on each day, per
    # cohort. A cohort's blocks sum to exactly the cycle length, so spreading
    # them by "most still owed" fills every day exactly and never leaves a
    # block with a remainder it cannot place.
    # Cohort = classes reachable through shared students, not year+campus:
    # senior classes now hold students from both sites.
    comp = {}
    adj = defaultdict(set)
    for a, ca in cls.items():
        for b2, cb in cls.items():
            if a < b2 and ca["students"] & cb["students"]:
                adj[a].add(b2); adj[b2].add(a)
    for code in cls:
        if code in comp: continue
        mark = len(comp); comp[code] = mark; stack=[code]
        while stack:
            cur = stack.pop()
            for nb in adj[cur]:
                if nb not in comp:
                    comp[nb] = mark; stack.append(nb)
    by_cohort = defaultdict(list)
    for i, b in enumerate(blocks):
        by_cohort[comp[b["classes"][0]]].append(i)

    plan = defaultdict(lambda: defaultdict(int))     # day -> block -> periods
    for cohort, idxs in by_cohort.items():
        left = {i: blocks[i]["periods"] for i in idxs}
        for day in sorted(day_slots):
            room = len(day_slots[day])
            taken = defaultdict(int)
            while room > 0:
                pick = [i for i in idxs if left[i] > 0 and taken[i] < cap]
                if not pick:
                    break
                i = max(pick, key=lambda i: (left[i], -taken[i], -i))
                plan[day][i] += 1
                taken[i] += 1
                left[i] -= 1
                room -= 1

    # Phase 2: turn that per-day plan into actual periods, which is where
    # rooms and staff are resolved between cohorts.
    ...
    for day in sorted(day_slots):
        for slot in day_slots[day]:
            for i in order:
                b = blocks[i]
                if plan[day][i] - used_today[day][i] <= 0:
                    continue
...
async def main():
    ...
    for k in sorted(per_cohort):
        print(f"    {k[0]:8} {str(k[1]):9} {per_cohort[k]}")
    print()
