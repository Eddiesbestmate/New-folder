...
def place(classes, slots, max_per_day, strategy="pack"):
    ...
    for code in order:
        ...
        while len(chosen) < want:
            ...
            elif strategy == "spread_days":
                # Keep each class spread across days, so the slots it leaves
                # behind stay spread too and the next class is not forced
                # into a day it already occupies.
                free.sort(key=lambda s: (per_day[s[0]], -load[s], s))
            else:  # spread_days_then_empty
                free.sort(key=lambda s: (per_day[s[0]], load[s], s))
...
async def main():
    ...
    for cap, strategy in ((2,"pack"), (3,"pack"), (2,"spread_days"),
                          (3,"spread_days"), (3,"spread_empty")):
        assigned, failures = place(classes, slots, cap, strategy)
        ...
        for code, (st, _, yr) in classes.items():
            for s in st:
                ...
                year_of[s] = yr
        print(f"cap={cap} {strategy:14} stranded={len(failures)}")
